...
/*   Updated: 2020/02/26 21:24:26 by vpetrova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (0 != read_obstacles(map, 0, ptr) || send->width == 0
		|| send->height < 1)
	{
		map_free(map);
		return (1);
...
int		read_header_and_first_line(t_map *map)
{
	...
	if (0 != map_init(map, send))
		return (1);
	return (read_header_and_first_line_part_2(map, ptr, &send));
	return (0);
}
...
int		file_get_map(const char *path, t_map *map)
{
	...
	return (file_get_map_part_2(map));
	close_file();
	return (0);
}
