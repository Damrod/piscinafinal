...
/*   Updated: 2020/02/26 20:56:08 by vpetrova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		map_init(t_map *map, t_map send)
{
	...
	if ((NULL == map->positions) || send.obstacle_symbol == send.empty_symbol ||
		send.empty_symbol == send.solution_symbol ||
		send.solution_symbol == send.obstacle_symbol)
		return (1);
...
int		map_is_obstacle(t_map *src, unsigned int row, unsigned int column)
{
	...
	if (column >= src->width)
		return (1);
	value = *(src->positions + (row * src->width) + column);
